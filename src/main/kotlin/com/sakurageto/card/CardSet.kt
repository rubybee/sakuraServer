...
object CardSet {
                            ...
                            if(madeAttack.kururuChange2X){
                                when(madeAttack.card_name){
                                    ...
                                    CardName.YUKIHI_FLUTTERING_SNOWFLAKE -> {
                                        if(game_status.version.isHigherThen(GameVersion.VERSION_8_2)){
                                            madeAttack.run { addRange(Pair(2, 4)); addRange(Pair(6, 7))}
                                        }
                                        else if(game_status.version.isHigherThen(GameVersion.VERSION_7_2)){
                                ...
                                when(madeAttack.card_name){
                                    ...
                                    CardName.YUKIHI_FLUTTERING_SNOWFLAKE -> {
                                        if(game_status.version.isHigherThen(GameVersion.VERSION_8_2)){
                                            madeAttack.run { addRange(Pair(1, 3)); addRange(Pair(5, 6))}
                                        }
                                        else if(game_status.version.isHigherThen(GameVersion.VERSION_7_2)){
                        ...
                        madeAttack.kururuChangeRangeUnder -> {
                            if(madeAttack.kururuChange2X){
                                when(madeAttack.card_name){
                                    ...
                                    CardName.YUKIHI_FLUTTERING_SNOWFLAKE -> {
                                        if(game_status.version.isHigherThen(GameVersion.VERSION_8_2)){
                                            madeAttack.run { addRange(Pair(-2, 0)); addRange(Pair(2, 3))}
                                        }
                                        else if(game_status.version.isHigherThen(GameVersion.VERSION_7_2)){
                                            madeAttack.run { addRange(Pair(-2, 0)); addRange(Pair(1, 4))}
                                        }
                                        else{
                                            madeAttack.run { addRange(Pair(-2, -1)); addRange(Pair(1, 3))}
                                        }
                                    }
                                    CardName.YUKIHI_SWAYING_LAMPLIGHT -> {
                                ...
                                when(madeAttack.card_name){
                                    ...
                                    CardName.YUKIHI_FLUTTERING_SNOWFLAKE -> {
                                        if(game_status.version.isHigherThen(GameVersion.VERSION_8_2)){
                                            madeAttack.run { addRange(Pair(-1, 1)); addRange(Pair(3, 4))}
                                        }
                                        else if(game_status.version.isHigherThen(GameVersion.VERSION_7_2)){
                            ...
                            when(madeAttack.card_name){
                                ...
                                CardName.YUKIHI_HIDDEN_FIRE_SLASH_CLAP_HANDS -> {
                                    if(game_status.version.isHigherThen(GameVersion.VERSION_8_1)){
                                        madeAttack.run { addRange(Pair(0, 1)); addRange(Pair(4, 6))}
                                    }
                                    else{
                                ...
                                CardName.YUKIHI_FLUTTERING_SNOWFLAKE -> {
                                    if(game_status.version.isHigherThen(GameVersion.VERSION_8_2)){
                                        madeAttack.run { addRange(Pair(0, 2)); addRange(Pair(4, 5))}
                                    }
                                    else if(game_status.version.isHigherThen(GameVersion.VERSION_7_2)){
    ...
    private fun shinraCardInit(){
        ...
        samraBanSho.addtext(Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.WHEN_ENCHANTMENT_DESTRUCTION_YOUR){ _, player, game_status, _ ->
            ...
            game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_SHINRA_SAMRA_BAN_SHO, -1))
            null
        })
                    ...
                    CommandEnum.SELECT_ONE -> {
                        ...
                        if(connectDive > 0){
                            if(game_status.processDamage(player.opposite(), CommandEnum.CHOOSE_LIFE, Pair(999, 2), false,
                                ...
                                game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KURURU_BIG_GOLEM, -1))
                                game_status.deckReconstruct(player, false)
                            }
                        }
                        else{
    ...
    private fun kururuCardInit(){
        elekittel.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { _, player, game_status, _ ->
            ...
            if(kikou.behavior >= 3 && kikou.reaction >= 2) {
                ...
                game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KURURU_ELEKITTEL, -1))
            }
            null
        })
        ...
        tornado.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { _, player, game_status, _ ->
            ...
            if(kikou.attack >= 2) {
                ...
                game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KURURU_TORNADO, -1))
            }
    ...
    suspend fun checkCanCardChange(player: PlayerEnum, game_status: GameStatus, original_card_number: Int,
    original_card_name: CardName, change_card_name: CardName): Boolean{
        if(game_status.getCardFrom(player, change_card_name, LocationEnum.ADDITIONAL_CARD) != null){
            if(game_status.getCardOwner(original_card_number) == player && checkCardName(original_card_number, original_card_name)){
                return true
            }
        }
        return false
        ...
        porucharto.addtext(Text(TextEffectTimingTag.CONSTANT_EFFECT, TextEffectTag.COST_BUFF) {card_number, player, game_status, react_attack->
            if((react_attack != null && react_attack.isItReact)){
                game_status.addThisTurnCostBuff(player, CostBuff(card_number, 1, BufTag.INSERT_IMMEDIATE, {_, _, card ->
                    (card.card_data.card_name == CardName.KORUNU_PORUCHARTO)}, {
                    4
                }))
            }
            null
    ...
    private fun yatsuhaCardInit(){
        ...
        twoLeapMirrorDivine.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.REACT_ATTACK_STATUS_CHANGE) { card_number, _, _, reactedAttack ->
                ...
                when(game_status.getStratagem(player)){
                    Stratagem.SHIN_SAN -> {
                        if(game_status.getPlayer(player.opposite()).normalCardDeck.size >= 3){
                            ...
                            game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_SHINRA_ZHEN_YEN, -1))
                        }
                        prepareStratagem(player, game_status)
                    }
                    Stratagem.GUE_MO -> {
    ...
    private suspend fun analyzeYour(player: PlayerEnum, game_status: GameStatus){
        ...
        game_status.popCardFrom(player, list[0], LocationEnum.COVER_CARD, true)?.let{
            ...
            if(it.card_data.card_type == CardType.ATTACK){
                ...
                game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KURURU_ANALYZE, -1))
            }
            else{
    ...
    private val flowingWallText = Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.MAKE_ATTACK) {card_number, player, game_status, _ ->
        ...
        game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_SAINE_FLOWING_WALL, -1))
    ...
    private fun honokaA1CardInit(){
        ...
        followTrace.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN){ _, player, game_status, _ ->
            ...
            while(true){
                firstCommand = game_status.requestAndDoBasicOperation(player, NUMBER_HONOKA_FOLLOW_TRACE)
                if(firstCommand != CommandEnum.SELECT_NOT){
                    break
                }
            }
            while(true){
                if( game_status.requestAndDoBasicOperation(player, NUMBER_HONOKA_FOLLOW_TRACE, hashSetOf(firstCommand)) != CommandEnum.SELECT_NOT){
                    break
                }
            }
            null
            ...
            while(true){
                when(game_status.receiveCardEffectSelect(player, NUMBER_KAMUWI_LAMP)){
                    CommandEnum.SELECT_ONE -> {
                        ...
                        game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KAMUWI_LAMP, -1))
                ...
                while(true){
                    when(game_status.receiveCardEffectSelect(player, NUMBER_KAMUWI_GRAVEYARD)){
                        CommandEnum.SELECT_ONE -> {
                            ...
                            game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_KAMUWI_GRAVEYARD, -1))
                            break
                        }
                        CommandEnum.SELECT_NOT -> {
                            break
    ...
    private val rururarariText = Text(TextEffectTimingTag.USED, TextEffectTag.WHEN_END_PHASE_YOUR){card_number, player, game_status, _ ->
        ...
        game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_RURURARARI_ADDITIONAL, -1))
        game_status.returnSpecialCard(player, card_number)
        null
    }
    ...
    private fun hatsumiA1CardInit(){
        ...
        wadanakaRoute.addtext(Text(TextEffectTimingTag.AFTER_DESTRUCTION, TextEffectTag.MOVE_TOKEN) { _, player, game_status, _ ->
            if(isTailWind(player, game_status)){
                ...
                game_status.processDamage(player.opposite(), CommandEnum.CHOOSE_LIFE, Pair(999, 1), false,
                    null, null, NUMBER_HATSUMI_WADANAKA_ROUTE)
                game_status.logger.insert(Log(player, LogText.END_EFFECT, NUMBER_HATSUMI_WADANAKA_ROUTE, -1))
            }
            null
        })
