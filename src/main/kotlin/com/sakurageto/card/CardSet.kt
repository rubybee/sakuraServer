...
object CardSet {
            ...
            while(true){
                ...
                if(selected == null){
                    ...
                    if(selected.size == 1){
                        ...
                        game_status.useCardNotFullPowerFrom(player, card, LocationEnum.COVER_CARD, false, null,
                        ...
                        game_status.useCardNotFullPowerFrom(player, secondCard, LocationEnum.DISCARD_YOUR, false, null,
                            isCost = true, isConsume = true)
                        break
                    }
                }
        ...
        tobikage.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.USE_CARD) ret@{_, player, game_status, react_attack ->
            while(true){
                ...
                if(selected.size == 1){
                    val selectNumber = selected[0]
                    val card = game_status.getCardFrom(player, selectNumber, LocationEnum.COVER_CARD)?: continue
                    game_status.useCardNotFullPowerFrom(player, card, LocationEnum.COVER_CARD, false, react_attack,
                        isCost = true, isConsume = true)
                    break
                }
                if(selected.size == 0){
    ...
    private fun shinraCardInit(){
        ...
        kiben.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.RUN_STRATAGEM){_, player, game_status, _ ->
            when(game_status.getStratagem(player)){
                Stratagem.SHIN_SAN -> {
                    game_status.deckToCoverCard(player, 3)
                        ...
                        if (list.isNotEmpty()){
                            ...
                            if (list.size == 1){
                                game_status.popCardFrom(player.opposite(), list[0], LocationEnum.DISCARD_YOUR, true)?.let {
                                    game_status.useCardNotFullPowerFrom(player, it, LocationEnum.DISCARD_OTHER, false, null,
                                        isCost = true, isConsume = true)
                                }?: continue
                                break
                            }
            ...
            while(true){
                ...
                if(selected.size == 1){
                    ...
                    while(true){
                        ...
                        if(nowCommand == CommandEnum.SELECT_ONE){
                            game_status.useCardNotFullPowerFrom(player, card, LocationEnum.HAND_OTHER, false, null,
                                isCost = true, isConsume = true)
                            break
                        }
                        else if(nowCommand == CommandEnum.SELECT_TWO){
                ...
                Stratagem.SHIN_SAN -> {
                    game_status.selectCardFrom(player, player, player, listOf(LocationEnum.DISCARD_YOUR, LocationEnum.YOUR_USED_CARD),
                        ...
                        game_status.getCardFrom(player, selected[0], LocationEnum.DISCARD_YOUR)?.also {
                            game_status.useCardNotFullPowerFrom(player, it, LocationEnum.DISCARD_YOUR, false, null,
                                isCost = true, isConsume = false
                            )
                        }?: game_status.getCardFrom(player, selected[0], LocationEnum.YOUR_USED_CARD)?.also {
                            game_status.useCardNotFullPowerFrom(player, it, LocationEnum.YOUR_USED_CARD, false, null,
                                isCost = true, isConsume = false
                            )
                        }?.let {card ->
    ...
    private val addCentrifugalLogText = Text(TextEffectTimingTag.CONSTANT_EFFECT, TextEffectTag.ADD_LOG) { card_number, player, game_status, _->
    ...
    private fun haganeCardInit(){
        ...
        centrifugalAttack.addtext(addCentrifugalLogText)
        ...
        fourWindedEarthquake.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.MOVE_CARD) {_, player, game_status, _ ->
            if (game_status.startTurnDistance + 1 < game_status.getAdjustDistance() || game_status.startTurnDistance - 1 > game_status.getAdjustDistance()) {
        ...
        wonMuRuyn.addtext(addCentrifugalLogText)
        ...
        ringABell.addtext(addCentrifugalLogText)
        ...
        grandMountainRespect.addtext(addCentrifugalLogText)
        grandMountainRespect.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.USE_CARD) {_, player, game_status, _ ->
            while(true){
                ...
                if(selected.size == 0) break
                    ...
                    val firstCard = game_status.getCardFrom(player, selected[0], LocationEnum.DISCARD_YOUR)?: continue
                    game_status.useCardNotFullPowerFrom(player, firstCard, LocationEnum.DISCARD_YOUR, false, null,
                        isCost = true, isConsume = true)
                    if(game_status.getEndTurn(player) || game_status.endCurrentPhase) break
                    if(selected.size == 2){
                        game_status.getCardFrom(player, selected[1], LocationEnum.DISCARD_YOUR)?.let { secondCard ->
                            game_status.useCardNotFullPowerFrom(player, secondCard, LocationEnum.DISCARD_YOUR, false, null,
                                isCost = true, isConsume = true)
                        }?: break
                    }
    ...
    private suspend fun regainer(effect_number: Int, card_number: Int, player: PlayerEnum, game_status: GameStatus, plusMinus: Int){
        while(true){
            ...
            if(list.size == 0) {
                ...
                game_status.useCardNotFullPowerFrom(player, card, location, false, null,
    ...
    private fun kururuCardInit(){
        elekittel.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, _ ->
            val kikou = getKikou(player, game_status)
                ...
                while(true){
                    ...
                    if(list.size == 1){
                        game_status.getCardFrom(player, list[0], LocationEnum.HAND)?.let { card ->
                            game_status.useCardNotFullPowerFrom(player, card, LocationEnum.HAND, false, null,
                                isCost = true, isConsume = true)
                        }
                        break
                    }
        ...
        tornado.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, _ ->
            val kikou = getKikou(player, game_status)
            if(kikou.attack >= 2) {
                var connectDive = 0
                for(card in game_status.getPlayer(player).usedSpecialCard.values){
        ...
        industria.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_CARD) {card_number, player, game_status, _->
            ...
            if(duplicateCardData != null){
                for(card in ownerPlayer.hand.values + ownerPlayer.normalCardDeck + ownerPlayer.discard + ownerPlayer.coverCard
        ...
        industria.addtext(Text(TextEffectTimingTag.USED, TextEffectTag.WHEN_THIS_CARD_RETURN){ card_number, player, game_status, _ ->
            val nowPlayer = game_status.getPlayer(player)
            for(card in nowPlayer.hand.values + nowPlayer.normalCardDeck + nowPlayer.discard + nowPlayer.coverCard
            ...
            while(true){
                ...
                if(list.size == 1){
                    game_status.getCardFrom(player.opposite(), list[0], LocationEnum.YOUR_USED_CARD)?.let { card ->
                        game_status.useCardNotFullPowerFrom(player, card, LocationEnum.OTHER_USED_CARD, false, null,
                            isCost = true, isConsume = false)
                    }
    ...
    private fun utsuroCardInit(){
        ...
        blackWave.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.WHEN_CHOOSE_AURA_DAMAGE) {_, player, game_status, _ ->
            while (true){
                ...
                { _, _ -> true }?: break
                if (list.size == 1){
                    game_status.popCardFrom(player.opposite(), list[0], LocationEnum.HAND, true)?.let{ card ->
                        game_status.insertCardTo(player.opposite(), card, LocationEnum.DISCARD_YOUR, true)
                    }
                    break
                }
        ...
        handFlower.addtext(Text(TextEffectTimingTag.USED, TextEffectTag.WHEN_DO_WIND_AROUND) ret@{card_number, player, game_status, _ ->
            ...
            while(true){
                when(game_status.receiveCardEffectSelect(player, NUMBER_HONOKA_HAND_FLOWER)){
                    ...
                    CommandEnum.SELECT_NOT -> {
                        val nowPlayer = game_status.getPlayer(player)
                        if(game_status.dust == 0 || nowPlayer.aura + nowPlayer.freezeToken == nowPlayer.maxAura) continue
                        break
                    }
                    else -> {
                        continue
    ...
    private suspend fun reviveDemise(player: PlayerEnum, game_status: GameStatus){
        ...
        nowPlayer.discard.filterNot { it.card_data.isItSpecial() }.forEach { card ->
            game_status.popCardFrom(player, card.card_number, LocationEnum.DISCARD_YOUR, false)?.let {
                game_status.insertCardTo(player, card, LocationEnum.OUT_OF_GAME, false)
            }
        }
    ...
    private fun utsuroA1CardInit(){
        ...
        deMise.addtext(Text(TextEffectTimingTag.AFTER_DESTRUCTION, TextEffectTag.MOVE_CARD) {_, player, game_status, _ ->
            ...
            for (cardNumber in otherPlayer.hand.keys.toList()){
    ...
    private fun korunuCardInit(){
        ...
        thornBush.addtext(Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.CAN_NOT_MOVE_TOKEN) { reason, player, game_status, _ ->
            var result = 1
            if(reason == Log.NORMAL_NAP_PROCESS && game_status.turnPlayer == player && game_status.getPlayer(player.opposite()).freezeToken >= 1) {
                while(true){
                ...
                if (list.size == 1){
                    ...
                    while (true){
                        when(game_status.receiveCardEffectSelect(player, NUMBER_YATSUHA_FOUR_LEAP_SONG)){
                            CommandEnum.SELECT_ONE -> {
                                if(game_status.useCardNotFullPowerFrom(player, card, location, false, null,
                                        isCost = true, isConsume = true)){
                                    if(card.card_data.sub_type == SubType.FULL_POWER){
    ...
    fun shinraA1CardInit(){
        ...
        sado.addtext(Text(TextEffectTimingTag.AFTER_DESTRUCTION, TextEffectTag.MAKE_ATTACK) ret@{card_number, player, game_status, react_attack->
            when(game_status.getStratagem(player)){
                Stratagem.SHIN_SAN -> {
                    if(game_status.addPreAttackZone(
                            player, MadeAttack(CardName.SHINRA_SA_DO,
    ...
    private fun kururuA1CardInit(){
        ...
        dauzing.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_CARD) ret@{_, player, game_status, _ ->
            ...
            ) { card, _ -> card.card_data.card_type != CardType.UNDEFINED }?: return@ret null
            game_status.getCardFrom(player.opposite(), list[0], LocationEnum.DISCARD_YOUR)?.let {
                val kikou = getKikou(player, game_status)
    ...
    private fun saineA2CardInit(){
        ...
        jeolChangJeolWha.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.NEXT_ATTACK_ENCHANTMENT) {_, _, game_status, react_attack ->
            react_attack?.afterAttackCompleteEffect?.add(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.WHEN_CHOOSE_AURA_DAMAGE) { _, in_player, in_game_status, _ ->
    ...
    private fun mizukiCardInit(){
        ...
        mizukiBattleCry.addtext(Text(TextEffectTimingTag.START_DEPLOYMENT, TextEffectTag.MIZUKI_BATTLE_CRY){_, player, game_status, _ ->
            ...
            for(card in cardPlayer.hand.values + cardPlayer.normalCardDeck + cardPlayer.discard + cardPlayer.coverCard
            ...
            if(!(game_status.logger.checkThisTurnUseCard(player) { card -> card != card_number })){
                game_status.selectCardFrom(player, player, player,
                    ...
                    game_status.getCardFrom(player, selected[0], LocationEnum.DISCARD_YOUR)?.let { card ->
                        game_status.useCardNotFullPowerFrom(player, card, LocationEnum.DISCARD_YOUR, false, null,
                            isCost = true, isConsume = true
                        )
                    }
                }
    ...
    private fun rairaA1CardInit(){
        ...
        jinPungJeCheonUi.addtext(Text(TextEffectTimingTag.USED, TextEffectTag.WHEN_THIS_CARD_RETURN) {_, player, game_status, _->
            //It presupposes that two jinPungJeCheonUi not exist in the used zone of one player's
    ...
    private val wildRoseText = Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.WHEN_END_PHASE_YOUR) { _, player, game_status, _ ->
        game_status.requestAndDoBasicOperation(player, NUMBER_MEGUMI_WILD_ROSE, hashSetOf())
        null
    }
    ...
    private fun megumiCardInit(){
        ...
        branchPossibility.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.REACT_ATTACK_CHANGE) { card_number, _, _, react_attack ->
            react_attack?.addAttackBuff(Buff(card_number, 1, BufTag.PLUS_MINUS_IMMEDIATE, {_, _, _ -> true },
                {attackPlayer, gameStatus, attack ->
            ...
            while(true){
                ...
                if(selected.size == 0){
                    ...
                    game_status.getCardFrom(player, selectNumber, LocationEnum.HAND)?.let { card ->
                        game_status.useCardNotFullPowerFrom(player, card, LocationEnum.HAND, false, react_attack,
                            isCost = true, isConsume = true)
    ...
    private fun honokaA1CardInit(){
        ...
        followTrace.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN){ _, player, game_status, _ ->
        ...
        followTrace.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN){ card_number, player, game_status, _ ->
    ...
    private fun renriCardInit(){
        ...
        ranararomirerira.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.USE_CARD) ret@{_, player, game_status, react_attack ->
            while(true){
                ...
                {card, _ -> card.card_data.sub_type != SubType.FULL_POWER && card.card_data.megami != MegamiEnum.RENRI
                        && card.card_data.card_type != CardType.UNDEFINED}
                        ...
                        when(game_status.receiveCardEffectSelect(player, NUMBER_RENRI_RA_NA_RA_RO_MI_RE_RI_RA)){
                            CommandEnum.SELECT_ONE -> {
                                ...
                                game_status.getCardFrom(player.opposite(), selectNumber, LocationEnum.HAND)?.let {
                                    game_status.useCardNotFullPowerFrom(player, it, LocationEnum.HAND_OTHER, false, react_attack,
                                        isCost = true, isConsume = true)
                                }
                                break
                            }
                            CommandEnum.SELECT_TWO -> {
                                val selectNumber = selected[0]
                                game_status.popCardFrom(player.opposite(), selectNumber, LocationEnum.HAND, true)?.let {
        ...
        renriTheEnd.addtext(Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.CAN_NOT_MOVE_TOKEN) { reason, player, game_status, _ ->
            if(game_status.turnPlayer == player && reason == Log.NORMAL_NAP_PROCESS) 0
            else 1
        })
    ...
    private fun yatsuhaA1CardInit(){
        ...
        pledge.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, react_attack->
            while (true) {
    ...
    private fun hatsumiA1CardInit(){
        ...
        wadanakaRoute.addtext(Text(TextEffectTimingTag.AFTER_DESTRUCTION, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, _ ->
    ...
    private val padmaCutDownEffectText = Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, _ ->
        game_status.processAllLacerationDamageCancelAble(player)
        game_status.addThisTurnAttackBuff(player, Buff(card_number, 1, BufTag.PLUS_MINUS_IMMEDIATE,
            {_, _, _ -> true},
            {_, _, attack ->
        ...
        if(game_status.addPreAttackZone(
                player, MadeAttack(CardName.SHISUI_PADMA_CUT_DOWN,
    ...
    private val vagueStoryV8Hypen2Text = Text(TextEffectTimingTag.CONSTANT_EFFECT, TextEffectTag.WHEN_START_PHASE_OTHER) { card_number, player, game_status, _ ->
        if(game_status.returnSpecialCard(player, card_number)){
            game_status.getConcentration(player.opposite())
        }
        null
    }
    ...
    fun v8hypen2CardInit(){
        ...
        ulooV8_2.addtext((Text(TextEffectTimingTag.START_DEPLOYMENT, TextEffectTag.MOVE_CARD) {_, player, game_status, _->
            game_status.selectCardFrom(player, player, player,
                ...
                for (cardNumber in selected){
                    game_status.popCardFrom(player, cardNumber, LocationEnum.COVER_CARD, true)?.let { card ->
        ...
        hiddenFireV8_2.addTextFold(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.CARD_DISCARD_PLACE_CHANGE) {_, player, game_status, _ ->
            game_status.changeUmbrella(player)
            null
        })
        hiddenFireV8_2.addTextUnfold(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.CARD_DISCARD_PLACE_CHANGE) {_, player, game_status, _ ->
            game_status.changeUmbrella(player)
            null
        })
            ...
            if(game_status.getPlayer(player).concentration >= 1 && game_status.canUseConcentration(player)){
                while(true){
                    when(game_status.receiveCardEffectSelect(player, NUMBER_SHINRA_SA_DO)){
                        CommandEnum.SELECT_ONE -> {
                            if(game_status.getPlayer(player).decreaseConcentration()){
                                setStratagemByUser(game_status, player)
                            }
                        }
                        CommandEnum.SELECT_NOT -> {
        ...
        sadoV8_2.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.MAKE_ATTACK) {card_number, player, game_status, _->
            sadoV8Hypen2MakeAttackEffect(player, game_status, card_number)
            null
        })
        ...
        deviceKururusikV8_2.addtext(Text(TextEffectTimingTag.USED, TextEffectTag.END_PHASE_EFFECT_WHEN_DUST_CHECK_PASS) { card_number, player, game_status, _ ->
        ...
        blackArmorV8_2.addtext(Text(TextEffectTimingTag.USING, TextEffectTag.MOVE_TOKEN) { card_number, player, game_status, react_attack ->
            val count = game_status.logger.countGetDamage(player)
            for(i in 1..count){
                while(true){
                    when(game_status.receiveCardEffectSelect(player, NUMBER_SHISUI_BLACK_ARMOR)){
                        CommandEnum.SELECT_ONE -> {
                            react_attack?.addAttackBuff(Buff(card_number, 1, BufTag.PLUS_MINUS_IMMEDIATE, {_, _, _ -> true },
                                {_, _, attack ->
                                    attack.lifePlusMinus(-1)
    ...
    private val riRaRuRiRaRo = CardData(CardClass.NORMAL, CardName.RENRI_RI_RA_RU_RI_RA_RO, MegamiEnum.RENRI, CardType.ENCHANTMENT, SubType.NONE)
    ...
    private fun renriA1CardInit(){
        deceptionFog.addtext(Text(TextEffectTimingTag.CONSTANT_EFFECT, TextEffectTag.TREAT_AS_DIFFERENT_CARD) ret@{card_number, player, game_status, react_attack ->
            ...
            if(otherPlayer.isDiscardHave(cardName) || otherPlayer.isDeckHave(cardName) || otherPlayer.isCoverHave(cardName)){
                while(true){
                    when(game_status.receiveCardEffectSelect(player.opposite(), NUMBER_RENRI_DECEPTION_FOG)){
                        CommandEnum.SELECT_ONE -> {
                            ...
                            game_status.popCardFrom(player, card_number, LocationEnum.HAND, true)
                            return@ret 1
                        }
                        CommandEnum.SELECT_NOT -> {
                ...
                delay(Random(System.currentTimeMillis()).nextLong() % 1000 + 1000)
                game_status.popCardFrom(player, card_number, LocationEnum.HAND, true)?.let {
                    game_status.insertCardTo(player, it, LocationEnum.PLAYING_ZONE_YOUR, true)
                }
                useAnotherCard(player, game_status, cardName, react_attack)
                ...
                game_status.popCardFrom(player, card_number, LocationEnum.PLAYING_ZONE_YOUR, true)?.let {sinSoo ->
                    ...
                    game_status.getPlayer(player).relic?.let { relic ->
                        if(relic.size > 0){
                            ...
                            game_status.popCardFrom(player, 0, LocationEnum.YOUR_DECK_TOP, false)?.let {
                                game_status.insertCardTo(player, it, LocationEnum.RELIC_YOUR, false)
                                game_status.popCardFrom(player, selected, LocationEnum.RELIC_YOUR, false)?.let {
                                    game_status.insertCardTo(player, it, LocationEnum.YOUR_DECK_TOP, false)
                                }
                            }
                        }
        ...
        falseWeapon.addtext(Text(TextEffectTimingTag.AFTER_ATTACK, TextEffectTag.NEXT_ATTACK_ENCHANTMENT) { card_number, player, game_status, _ ->
            game_status.addThisTurnAttackBuff(player, Buff(card_number, 1, BufTag.PLUS_MINUS,
                { condition_player, condition_game_status, condition_attack ->
                    condition_game_status.logger.checkThisCardUseWhen(condition_player, condition_attack.card_number) == 3
                }, { _, _, attack ->
                    attack.lifePlusMinus(1)
                })
            )
        ...
        riRaRuRiRaRo.addtext(Text(TextEffectTimingTag.CONSTANT_EFFECT, TextEffectTag.ADJUST_NAP_WHEN_SOME_CONDITION) {card_number, player, game_status, _ ->
            game_status.getCardFrom(player, card_number, LocationEnum.PLAYING_ZONE_YOUR)?.let {
        ...
        riRaRuRiRaRo.addtext(Text(TextEffectTimingTag.USED, TextEffectTag.WHEN_END_PHASE_YOUR) { card_number, player, game_status, _ ->
        ...
        riRaRuRiRaRo.addtext(Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.CHOJO_DAMAGE_CHANGE_OTHER) { _, player, game_status, _->
            21
        })
        riRaRuRiRaRo.addtext(Text(TextEffectTimingTag.IN_DEPLOYMENT, TextEffectTag.RIRARURIRARO_EFFECT) { _, player, game_status, _->
            1
        })
    }
